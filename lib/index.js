...
function SessionStore(options, connection) {
	...
	function reconnect(quiet) {
		...
		connection.connect(function (err) {
			console.log('delay', ((typeof options.maxReconnectAttempts !== "undefined" && reconnectCount > options.maxReconnectAttempts) || (reconnectCount > MAX_RECONNECTION_ATTEMPTS)) ? OUTAGE_RECONNECT_DELAY : options.reconnectDelay || RECONNECT_DELAY);
			if (err) {
				setTimeout(function () {
					reconnectCount++;
					reconnect();
				}, ((typeof options.maxReconnectAttempts !== "undefined" && reconnectCount > options.maxReconnectAttempts) || (reconnectCount > MAX_RECONNECTION_ATTEMPTS)) ? options.outageReconnectionDelay || OUTAGE_RECONNECT_DELAY : options.reconnectDelay || RECONNECT_DELAY);
			} else {
