...
function SessionStore(options, connection) {
	...
	var expressSessionStore = new express.session.Store();
	 ...
	 */
	expressSessionStore.sync = function sync(callback) {
		fs.readFile(__dirname + '/../schema.sql', 'utf-8', function (error, sql) {
			connection.query(sql, function (error) {
				...
				expressSessionStore.setExpirationInterval();
				if (callback) {
					callback();
				}
			});
	 ...
	 */
	expressSessionStore.get = function get(session_id, callback) {
	 ...
	 */
	expressSessionStore.set = function set(session_id, data, callback) {
	 ...
	 * @param  {Function} callback   
	 */
	expressSessionStore.destroy = function destroy(session_id, callback) {
	 ...
	 * @param  {Function} callback
	 */
	expressSessionStore.length = function length(callback) {
	 ...
	 * @param  {Function} callback 
	 */
	expressSessionStore.clear = function clear(callback) {
	 ...
	 * @param  {Function} callback 
	 */
	expressSessionStore.clearExpiredSessions = function clearExpiredSessions(callback) {
		...
		connection.query(sql, params, function (error, rows) {
			...
			var l = rows.length;
			while (l--) {
				expressSessionStore.destroy(rows[l].session_id);
			}
			if (callback) {
				callback();
			}
	...
	expressSessionStore.setExpirationInterval = function setExpirationInterval(interval) {
		clearInterval(expirationInterval);
		expirationInterval = setInterval(expressSessionStore.clearExpiredSessions, options.checkExpirationInterval = (interval || options.checkExpirationInterval || CHECK_EXPIRATION_INTERVAL));
	};
	expressSessionStore.sync();
	expressSessionStore.connection = connection;
